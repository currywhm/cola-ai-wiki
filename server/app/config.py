...
class Settings(BaseSettings):
    ...
    # 命令沙箱可用性：None = 按官方 dsh-sandbox-local 的探测口径自动判断（Linux 探 bwrap、
    # macOS 探 sandbox-exec）；显式 true/false 则强制覆盖。不可用时，系统提示会告诉模型
    # "本环境没有命令行"，免得它把步骤烧在注定失败的 bash 上（SandboxUnavailableError）。
    harness_shell_available: bool | None = None
